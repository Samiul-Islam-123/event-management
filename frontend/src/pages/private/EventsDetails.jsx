            ...
            <Nav />
            {
                details && (<>
                    <div className=' w-full h-screen relative bg-black flex items-center px-12'>
                        <img src={details.poster} alt="" className='absolute left-0 w-full h-full object-cover opacity-50' />

                        <div className='flex w-[80vw] md:w-[40vw] z-10 flex-col text-white'>
                            <div className=' md:mb-5 wifull mb-2'>
                                <h1 className=' md:text-7xl text-4xl font-bold'>{details.name}</h1>
                            </div>
                            <div className=' md:text-2xl text-xl'>
                                <p className=" font-medium">
                                    Organized By <span className=" text-[#E167FF]">{details.organizer.username}</span>
                                </p>
                            </div>
                            <div className="flex md:flex-row flex-col md:items-center  md:gap-3 gap-1 md:mb-8 mb-3">
                                <div className="flex items-center gap-1"><CiCalendar size={20} />
                                    <p className=" font-semibold">{new Date(details.date).toLocaleDateString('en-US', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                        })}</p></div>
                                ...
                                <div className="flex items-center gap-1"><CiLocationOn size={20} />
                                    <p className=" font-semibold">{details.location}</p></div>
                            </div>

                            <div className=' w-fit h-[10vh] bg-white rounded-lg flex items-center justify-between px-6'>
                                <h1 className=' text-[#E167FF] text-3xl font-bold mr-5'>${details.price}</h1>
                    ...
                    <div className='z-10 w-full flex h-screen px-12 py-5'>
                        <div className=' w-1/2' >
                            ...
                            <div >
                                <h1 className=' md:text-8xl text-5xl font-semibold text-[#E167FF] qwigley-regular mb-3'>{details.name}</h1>

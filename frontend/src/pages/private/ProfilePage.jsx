...
function ProfilePage() {
  ...
  const fetchBoughtTickets = async () => {
    try {
      console.log(`${import.meta.env.VITE_API_URL}/ticket/bought${localStorage.getItem('user_id')}`)
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/ticket/bought/${localStorage.getItem('user_id')}`);
      console.log(response.data.boughtTickets)
        ...
        <div className="flex flex-wrap gap-6">
          ...
          { boughtTickets ? (
            boughtTickets.map(ticket => (
               <EventCard key={ticket.id} {...ticket.event} />
              // <>
